...
state of the environment to two possible outputs:

1.Recommended action: A probability value for each action in the action space. The part 
  of the agent responsible for this output is called the actor.
...
to move the cart. It is rewarded for every time step the pole remains upright. Therefore, 
the agent must learn to keep the pole from falling over.
...
while True:  # Run until solved
    ...
    with tf.GradientTape() as tape:
        ...
        for log_prob, value, ret in history:
            # At this point in history, the critic estimated that we would get a total reward 
            # = `value` in the future. We took an action with log probability of log_prob and 
            # ended up recieving a total reward = ret. The actor must be updated so that it 
            # predicts an action that leads to high rewards (compared to critic's estimate) 
            # with high probability.
            diff = ret - value
            actor_losses.append(-log_prob * diff)  # actor loss

            # The critic must be updated so that it predicts a better estimate of the future 
            # rewards.
