...
We implement the AdaMatch algorithm, proposed in [AdaMatch: A Unified Approach to Semi-Supervised 
Learning and Domain Adaptation](https://arxiv.org/abs/2106.04732) by Berthelot et al. It sets a 
new state-of-the-art in unsupervised domain adaptation (as of June 2021). AdaMatch is particularly 
interesting because it unifies semi-supervised learning (SSL) and unsupervised domain adaptation
(UDA) under one framework. It provides a way to perform semi-supervised domain adaptation (SSDA).

The example requires TensorFlow 2.5 or higher, as well as TensorFlow Models, which can be installed 
with using the following command:

Miniconda: 
$ conda intall pip
$ pip install tf-models-official==2.10.0
$ conda install -c conda-forge keras-cv

Before we proceed, let's review a few preliminary concepts underlying this example.

## Preliminaries

In semi-supervised learning (SSL), we use a small amount of labeled data to train models on a 
bigger unlabeled dataset. Popular semi-supervised learning methods for computer vision include 
[FixMatch](https://arxiv.org/abs/2001.07685), [MixMatch](https://arxiv.org/abs/1905.02249),
[Noisy Student Training](https://arxiv.org/abs/1911.04252), etc. You can refer to [this example]
(https://keras.io/examples/vision/consistency_training/) to get an idea of what a standard SSL 
workflow looks like.

In unsupervised domain adaptation, we have access to a source labeled dataset and a target 
unlabeled dataset. Then the task is to learn a model that can generalize well to the target 
dataset. The source and the target datasets vary in terms of distribution. The following figure 
provides an illustration of the idea. In the present example, we use the [MNIST dataset]
(http://yann.lecun.com/exdb/mnist/) as the source dataset, while the target dataset is [SVHN]
(http://ufldl.stanford.edu/housenumbers/), which consists of images of house numbers. Both 
datasets have various varying factors in terms of texture, viewpoint, appearence, etc.: their 
domains, or distributions, are different from one another.

## Data augmentation utilities

A standard element of SSL algorithms is to feed weakly and strongly augmented versions of the same 
images to the learning model to make predictions consistent. For strong augmentation, [RandAugment]
(https://arxiv.org/abs/1909.13719) is a standard choice. For weak augmentation, we will use hori-
zontal flipping and random cropping.

## Data loading utilities

Here's what a single image batch looks like: ![](https://i.imgur.com/aver8cG.png)
...
1.We first retrieve the weakly and strongly augmented pairs of images from the source and target 
  datasets.
...
3.We run two forward passes through the model:
  i.The first forward pass uses the concatenated copy obtained from 2.i. In this forward pass, 
     the [Batch Normalization](https://arxiv.org/abs/1502.03167) statistics are updated.
  ii.In the second forward pass, we only use the concatenated copy obtained from the above 2.ii. 
     Batch Norm layers are run in inference mode.
4.The respective logits are computed for both the forward passes.
5.The logits go through a series of transformations, introduced in the paper (which we will discuss 
  shortly).
...
In AdaMatch, we perform two forward passes, and only one of them is respsonsible for updating the 
Batch Norm statistics. This is done to account for distribution shifts in the target dataset. In 
the other forward pass, we only use the source sample, and the Batch Norm layers that are run in 
inference mode. Logits for the source samples (weakly and strongly augmented versions) from these 
two passes are slightly different from one another because of how Batch Norm layers are run. Final 
logits for the source samples are computed by linearly interpolating between these two different 
pairs of logits. This induces a form of consistency regularization. This step is referred to as 
random logit interpolation.

Distribution alignment is used to align the source and target label distributions. This further 
helps the underlying model learn domain-invariant representations. In case of unsupervised domain 
adaptation, we don't have access to any labels of the target dataset. This is why pseudo labels 
are generated from the underlying model.

The underlying model generates pseudo-labels for the target samples. It's likely the model would 
make faulty predictions. Those can propagate back as we make progress in the training, and hurt 
the overall performance. To compensate for that, we filter the high-confidence predictions based 
on a threshold (hence the use of `mask` inside compute_loss_target()`). In AdaMatch, the threshold 
is relatively adjusted which is whyit is called relative confidence thresholding.

For more details on these methods and to know how each of them contribute please refer to [the 
paper](https://arxiv.org/abs/2106.04732).

About `compute_mu()`:

Rather than using a fixed scalar quantity, a varying scalar is used in AdaMatch. It denotes the 
weight of the loss contibuted by the target samples. Visually, the weight scheduler look like:

![](https://i.imgur.com/dG7i9uH.png)

This scheduler increases the weight of the target domain loss from 0 to 1 for the first half of 
the training. Then it keeps that weight at 1 for the second half of the training.

## Instantiate a Wide-ResNet-28-2

The authors use a [WideResNet-28-2](https://arxiv.org/abs/1605.07146) for the dataset pairs we are 
using in this example. Most of the following code has been referred from that the following model 
has a scaling layer inside it that scales the pixel values to [0, 1].
[this script](https://github.com/asmith26/wide_resnets_keras/blob/master/main.py). 

Please note we can now instantiate a Wide ResNet model like the above one. Note that the purpose 
of using aWide ResNet here is to keep the implementation as close to the original one as possible.

## AdaMatch model training and compiling

With more training, this score improves. When this same network is trained with standard classifi-
cation objective, it yields an accuracy of 7.20% which is significantly lower than what we got with 
AdaMatch. Please check out [this notebook] to learn more about the hyperparameters and other experi-
mental details.
...
def weak_augment(image, source=True):
    ...
    # MNIST images are grayscale, this is why we first convert them to RGB images.
    if source:
...
# The suffixes of w and s denote weak and strong respectively.
...
class AdaMatch(keras.Model):
    ...
    # This is a warmup schedule to update the weight of the loss contributed by 
    # the target unlabeled samples. More on this in the text.
    def compute_mu(self):
...
# Stack residual units on the same stage
