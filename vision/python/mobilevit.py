...
Description: MobileViT for image classification with combined benefits of convolutions 
and Transformers.

## Introduction

We implement the MobileViT ([Mehta et al.](https://arxiv.org/abs/2110.02178)), which 
combines the benefits of Transformers([Vaswani et al.](https://arxiv.org/abs/1706.03762))
...
* Second, they get unfolded into another vector with shape `(p, n, num_channels)`, where `p` 
  is the area of a small patch, and `n` is `(h * w) / p`. We end with `n` non-overlapping 
  patches.
...
Vectors A and B are passed through two more convolutional layers to fuse the local and 
global representations. Notice how the spatial resolution of the final vector remains
...
epochs = 30 # 6 minutes per epochs
...
# Convert to TFLite for post-training dynamic-range quantization.
