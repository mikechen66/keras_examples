...
their performance can be improved. For example, in the following weblink. 

[this work](https://arxiv.org/abs/1811.12231);
...
common perception. But does this belief always apply especially when it comes to improve
...
(https://arxiv.org/abs/2103.09950v1), Talebi et al.

They show that if we try to optimize the perceptual quality of the images for the vision 
models rather than the human eyes, the performance can be improved. They investigate the 
following question:
...
The figure below (courtesy: [Learning to Resize Images for Computer Vision Tasks] presents 
the structure of the learnable resizing module:
...
interpolation. For more details, you can check out [this repository]
...
Now, a question worth asking here isn't the improved accuracy simply a consequence of 
adding more layers (the resizer is a mini network after all) to the model, compared to
the baseline?_
...
like [Squeeze and Excitation (SE) blocks](https://arxiv.org/abs/1709.01507), [Global Context 
(GC) blocks](https://arxiv.org/pdf/1904.11492) also add a few parameters to an existing 
network but they are known to help a network process information in systematic ways to 
