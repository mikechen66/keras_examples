...
Description: Building a near-duplicate image search utility using deep learning and 
...
Search, etc. In this example, we build a similar image search utility using [Locality 
Sensitive Hashing]
...
This kind of search engine is known as a _near-duplicate (or near-dup) image detector_.
We will also look into optimizing the inference performance of our search utility on 
GPU using [TensorRT](https://developer.nvidia.com/tensorrt).

There are other examples under [keras.io/examples/vision] that are worth checking out 
in this regard(https://keras.io/examples/vision).
...
Finally, this example uses the following resource as a reference and as such reuses 
...
To keep the run time of the example short, we will be using a subset of 1,000 images 
from the `tf_flowers` dataset (available through [TensorFlow Datasets]
(https://www.tensorflow.org/datasets/catalog/tf_flowers)) to build our vocabulary.

Load a pre-trained model

We load an image classification model that was trained on the 'tf_flowers' dataset. 
85% of the total images were used to build the training set. For more details on the 
training, refer to [this notebook]
(https://github.com/sayakpaul/near-dup-parser/blob/main/bit-supervised-training.ipynb).

The underlying model is a BiT-ResNet (proposed in [Big Transfer (BiT): General Visual 
Representation Learning](https://arxiv.org/abs/1912.11370)). The BiT-ResNet family of 
...
To retrieve similar images given a query image, we need to first generate vector repre-
sentations of all the images involved. We do this via an embedding model that extracts 
output features from the pretrained classifier and normalizes feature vectors.

Take note of the normalization layer inside the model. It is used to project the repre-
sentation vectors to the space of unit-spheres.

The shape of the vectors coming out of `embedding_model` is `(2048,)`, and considering 
practical aspects (storage, retrieval performance, etc.) it is quite large. So, there 
arises a need to reduce the dimensionality of the embedding vectors without reducing 
their information content. This is where *random projection* comes into the picture.
It is based on the principle that if the distance between a group of points on a given 
plane is _approximately_ preserved, the dimensionality of that plane can further be 
reduced.
...
We first do the GPU wam-up and proceed to build the master LSH table with `embedding_model`.
...
For NVIDIA-based GPUs, the [TensorRT framework] can be used to dramatically enhance the 
inference latency by using various model optimization techniques like pruning, constant 
folding, layer fusion, and so on. Here we use the `tf.experimental.tensorrt` module to 
...
* `precision_mode` defines the numerical precision of the operations in the to-be-converted 
  model.
* `maximum_cached_engines` specifies the maximum number of TRT engines that will be cached 
  to handle dynamic operations (operations with unknown shapes).

To learn more about the other options, refer to the [official documentation]
...
Notice the difference in the wall time which is 13.1 seconds. Earlier, with the unoptimized 
model it was 54.1 seconds.

We take a closer look into one of the hash tables and get an idea of how they are represented.

## Visualize results on validation images

In this section we will first writing a couple of utility functions to visualize the similar 
image parsing process. Then we will benchmark the query performance of the models with and 
without optimization. We take 100 images from the validation set for testing purposes. There 
are two ways: No TRT and TRT model. 
...
bit_model = tf.keras.models.load_model("/home/mike/datasets/flower_model_bit_0.96875")
...
# Pack the utilities to have multiple hash tables.
class LSH:
...
# Serialize the embedding model as a SavedModel.
...
# First: take 100 images from the validation set for testing purposes.
validation_images = []
validation_labels = []
...
# Second: write our visualization utilities.
