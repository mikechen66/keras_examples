...
Author: [Aritra Roy Gosthipaty](https://twitter.com/arig23498), [Sayak Paul]
...
It includes extracting the encoder model along with other layers, preparation datasets 
for linear probing and performing linear probing. With regard to the first one, we use 
average pooling to extract learned representations from the MAE encoder. Another approach 
would be to use a learnable dummy token inside the encoder during pretraining (resembling 
the [CLS] token). Then we can extract representations from that token during the down-
stream tasks.
...
the encoder architecture and [trained it from scratch] in a fully supervised manner. It 
gave us ~76% test top-1 accuracy. The authors of MAE demonstrates strong performance on 
...
(https://arxiv.org/abs/2111.09886). Finally, as a fun fact, we, the  authors of this 
example also explored the idea of ["reconstruction as a pretext task"]
(https://i.ibb.co/k5CpwDX/image.png) in 2020 but we could not prevent the network from 
representation collapse, and hence we did not get strong downstream performance.

We would like to thank [Xinlei Chen](http://xinleic.xyz/) (one of the authors of MAE) 
for helpful discussions. We are grateful to [JarvisLabs](https://jarvislabs.ai/) and
[Google Developers Experts](https://developers.google.com/programs/experts/) program for 
helping with GPU credits.
"""
...
# See the masking process in an action on a sample image.
...
## MLP serves as the fc ffn of the transformer architecture.
...
# Visualization callback - Taking a batch of test inputs to measure model's progress.
...
# Learning rate scheduler: Some code is taken from:
